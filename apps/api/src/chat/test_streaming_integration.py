...
class TestDifyStreamingIntegration:
    ...
    async def dify_service(self):
        ...
        service = DifyService(
            base_url="https://test.dify.com/v1",
    ...
    async def test_streaming_request_success(self, mock_request, dify_service, mock_streaming_response):
        """测试成功的流式请求"""
    ...
    @patch('aiohttp.ClientSession.request')
    async def test_send_message_stream_mode(self, mock_request, dify_service, mock_streaming_response):
    ...
    @patch('aiohttp.ClientSession.request')
    async def test_collect_stream_response(self, mock_request, dify_service, mock_streaming_response):
        """测试收集完整流式响应"""
    ...
    async def test_streaming_error_handling(self, mock_request, dify_service):
        ...
        with pytest.raises(Exception):  # 应该抛出 AuthenticationError
            stream_gen = dify_service._make_streaming_request("POST", "/chat-messages", {})
