...
class DifyService:
    ...
    async def _make_streaming_request(
        ...
        log_api_call(f"{method} {endpoint} (streaming)", {"params": params, "data": data})
                    ...
                    for line in lines[:-1]:
                        ...
                        if line.startswith('data: '):
                            try:
                                ...
                                logger.warning(f"Failed to parse SSE data: {json_str}, error: {e}")
                                continue
                            except Exception as e:
                                logger.error(f"Error processing SSE line: {line}, error: {e}")
                                continue
                ...
                if buffer.strip():
                    ...
                    if line.startswith('data: '):
                        try:
                            ...
                            logger.warning(f"Failed to parse final SSE data: {json_str}, error: {e}")
    ...
    async def send_message_with_retry_stream(
        ...
        if last_exception:
            ...
            raise DifyServiceException("All retry attempts failed for streaming request")
