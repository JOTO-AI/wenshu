...
async def example_streaming_chat():
    ...
    dify_service = DifyService(
        base_url="https://dify.jototech.cn/v1", api_key="your-api-key-here", timeout=30
    )
    ...
    try:
        ...
        stream_generator = await dify_service.send_message(
            query="你好，请介绍一下你自己", stream=True, user="test_user"
        )
        ...
        async for event in stream_generator:
            ...
            if event_type == "workflow_started":
                ...
                print(
                    f"  节点开始: {parsed_event.get('title')} ({parsed_event.get('node_type')})"
                )

            elif event_type == "node_finished":
                print(
                    f"  节点完成: {parsed_event.get('title')}, 耗时: {parsed_event.get('elapsed_time')}s"
                )
        ...
        # 收集完整响应
        complete_response = await dify_service.collect_stream_response(
            stream_generator2
        )
...
async def example_event_filtering():
    ...
    try:
        ...
        stream_generator = await dify_service.send_message(
            query="解释一下机器学习的基本概念", stream=True, user="test_user"
        )
